...
<script type="text/javascript">
function create() {
	var sxword = document.getElementById("sxword").value;
	if (sxword == "") {
		alert("输入不能为空");
	}
	else{
		$.ajax({
			method : "POST",
			async : true,
			url : "StudentExchangeController.mvc",
			...
			success : function(data) {
				if (data == "success") {
					alert("输入成功");
					top.location = "studentexchange.jsp";
				}
			}
		})
	}
...
<body class="withvernav">
	<div class="bodywrapper">
		...
		<div style="position: absolute; top: 200px; left: 600px">
			<c:if test="${sessionScope.astudent==0}">
				没有交流
			</c:if>
			<c:if test="${sessionScope.astudent != 0}">
				<c:forEach var="up" items="${sessionScope.studentex1}">
					${up.SXnum}-${up.SXname}:${up.SXword}<br>
				</c:forEach>
			</c:if>
			<br>
			${sessionScope.snum}请发表你的言论：
			<br>
				<input id="sxword" name="sxword" type="text" placeholder="请输入你想说的话" style="width: 250px;">
				<br>
