...
<script type="text/javascript">
function updatethisa(snum){
	var psw = document.getElementById("thispsw").value;
	if(psw==''){
		alert("修改的密码不能为空");
	}else{
		$.ajax({
			method : "POST",
			async : true,
			url : "UpdateSpecialStuPswController.mvc",
...
<body class="withvernav">
	...
	<div class="bodywrapper">
		...
		<div style="position: absolute;top: 200px;left: 600px;">
			学生学号:${sessionScope.needtochange.snum}<br/>
			学生密码:${sessionScope.needtochange.spassword}<br/>
			修改当前密码:<input type="text" id="thispsw" name="thispsw"><br/>
			<button onclick="updatethisa(${sessionScope.needtochange.snum})">提交</button>
		</div>
		<!-- centercontent -->
