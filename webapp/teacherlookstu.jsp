...
function updatethisa(snum){
		$.ajax({
			method : "POST",
			async : true,
			url : "UpdateSpecialStuInfoController.mvc",
			...
			success : function(data) {
				if (data == "success") {
					top.location = "teaupdatespecialstuinfo.jsp";
				}
			}
		})
	}
...
<body class="withvernav">
	...
	<div class="bodywrapper">
		...
		<div class="centercontent" style=" text-align: center;">
			<c:if test="${sessionScope.reallyhavestutochange==0}">暂无学生</c:if>
			<c:if test="${sessionScope.reallyhavestutochange!=0}">
				<table style="border: 1px;margin-left: auto; margin-right: auto;">
					<tr>
						<th>学生学号</th>
						<th>修改按钮</th>
					</tr>
					<c:forEach var="up" items="${sessionScope.allofstudent}">
						<tr>
							<th>${up.SInum}</th>
							<th><button onclick="updatethisa(${up.SInum})">修改</button></th>
						</tr>
					</c:forEach>
				</table>
			</c:if>
